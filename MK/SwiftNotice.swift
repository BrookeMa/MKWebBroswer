...
//    }
    ...
    func noticeSaveSuccess(_ text: String, autoClear: Bool = true, autoClearTime: Int = 1) {
        SwiftNotice.showNoticeSuccess(text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    func noticeSaveFail(_ text: String, autoClear: Bool = true, autoClearTime: Int = 1) {
        SwiftNotice.showNoticeFail(text: text, autoClear: autoClear, autoClearTime: autoClearTime)
...
class SwiftNotice: NSObject {
    ...
    static func noticeOnStatusBar(_ text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        window.frame.origin.y = frame.origin.y - 20
        view.frame.origin.y = frame.origin.y - 20
        ...
        if autoClear {
            let selector = #selector(SwiftNotice.hideTopNotice(_:))
...
//    }
    ...
    static func showNoticeSuccess(text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        let checkmarkView = UIImageView(image: UIImage(named: "mk_success_mark"))
        checkmarkView.frame = CGRect(x: 0, y: 20, width: 130, height: 55)
        checkmarkView.contentMode = .scaleAspectFit
        mainView.addSubview(checkmarkView)
        
        let label = UILabel(frame: CGRect(x: 0, y: 90, width: 130, height: 20))
    ...
    static func showNoticeFail(text: String, autoClear: Bool, autoClearTime: Int) {
        let frame = CGRect(x: 0, y: 0, width: 130, height: 130)
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        ...
        let checkmarkView = UIImageView(image: UIImage(named: "mk_fail_mark"))
        checkmarkView.frame = CGRect(x: 0, y: 20, width: 130, height: 55)
        checkmarkView.contentMode = .scaleAspectFit
        mainView.addSubview(checkmarkView)
    ...
    static func hideTopNotice(_ sender: AnyObject) {
        if let window = sender as? UIWindow {
            if let index = windows.index(where: { (item) -> Bool in
                return item == window
            }) {
                UIView.animate(withDuration: 0.6, animations: {
                    windows[index].frame.origin.y = -20
